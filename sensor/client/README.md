# Overview
The peripheral device has been tested with an *ESP32 DevKitc V4* (The new design uses *ESP32C3 WROOM 02*), is writen in [MicroPython](https://micropython.org/download/), and is designed to run on [bare metal](https://www.techopedia.com/definition/2153/bare-metal). The code makes use of its `bluetooth` and `uasyncio` libraries. The code also relies on 
