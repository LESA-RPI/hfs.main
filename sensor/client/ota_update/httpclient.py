...
class HttpClient:
    ...
    def request(self, method, url, data=None, json=None, file=None, custom=None, saveToFile=None, headers={}, stream=None):
        ...
        try:
            ...
            # add user agent
            s.write(b'User-Agent: {}\r\n'.format('Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/56.0.2924.87 Safari/537.36'))
            if json is not None:
                assert data is None
                import ujson
        ...
        if redirect:
            ...
            if status in [301, 302, 303]:
                return self.request('GET', url=redirect, data=data, json=json, file=file, custom=custom, saveToFile=saveToFile, headers=headers, stream=stream)
            else:
                return self.request(method, redirect, url=redirect, data=data, json=json, file=file, custom=custom, saveToFile=saveToFile, headers=headers, stream=stream)
        else:
