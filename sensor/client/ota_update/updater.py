...
class OTAUpdater:
    ...
    def __init__(self, github_repo, update_sub_dir=False, github_src_dir='', module='', main_dir='main', new_version_dir='next', secrets_file=None, headers={}, dev_version_url=''):
        ...
        self.dev_version_url = dev_version_url
        self.update_sub_dir = update_sub_dir
    ...
    def install_update_if_available(self) -> bool:
        ...
        (current_version, latest_version) = self._check_for_new_dev_version()
        if latest_version > current_version:
    ...
    def dev_install_update_if_available(self) -> bool:
        """This method will immediately install the latest version of a repo if out-of-date.
        ...
        (current_version, latest_version) = self._check_for_new_dev_version()
        if latest_version > current_version:
            print('[INFO] Updating to version {}...'.format(latest_version))
            self._create_new_version_file(latest_version)
            self._dev_install_new_version(latest_version)
            return True
    ...
    def _dev_install_new_version(self, version):
        print('[INFO] Installing version {}'.format(version))
        self._install_all_files(version)
        print('[INFO] Version {} installed'.format(version))
            ...
            if file['type'] == 'file':
                ...
                if self.update_sub_dir:
                    ...
                    try:
                        ...
                        pass
                    self._install_all_files(version, sub_dir + file['name'])
                else:
                    print('[INFO] Skipping directory', path)
            gc.collect()
        file_list.close()
    ...
    def _rmtree(self, directory):
        ...
        for entry in os.ilistdir(directory):
            ...
            if is_dir:
                ...
                print("[INFO] Removing ", directory + '/' + entry[0])
                os.remove(directory + '/' + entry[0])
        print("[INFO] Removing ", directory)
        os.rmdir(directory)
